import { SHOULD_AUTOBATCH, applyPatches, combineReducers, createAction, createAsyncThunk, createSelector, createSlice, enablePatches, isAction, isAllOf, isAnyOf, isAsyncThunkAction, isDraft, isDraftable, isFulfilled, isPending, isPlainObject, isRejected, isRejectedWithValue, nanoid, original, prepareAutoBatched, produce, produceWithPatches, weakMapMemoize } from "./redux-toolkit.modern-DdR6sdOA.js";
...
function calculateProvidedBy(description, result, error, queryArg, meta, assertTagTypes) {
	if (isFunction(description)) return description(result, error, queryArg, meta).filter(isNotNullish).map(expandTagDescription).map(assertTagTypes);
	if (Array.isArray(description)) return description.map(expandTagDescription).map(assertTagTypes);
	return [];
}
...
function buildSlice({ reducerPath, queryThunk, mutationThunk, serializeQueryArgs, context: { endpointDefinitions: definitions, apiUid, extractRehydrationInfo, hasRehydrationInfo }, assertTagType, config }) {
	...
	const querySlice = createSlice({
		...
		extraReducers(builder) {
			builder.addCase(queryThunk.pending, (draft, { meta, meta: { arg } }) => {
				...
				for (const [key, entry] of Object.entries(queries)) if (entry?.status === "fulfilled" || entry?.status === "rejected") draft[key] = entry;
			});
		}
	});
	const mutationSlice = createSlice({
		...
		extraReducers(builder) {
			builder.addCase(mutationThunk.pending, (draft, { meta, meta: { requestId, arg, startedTimeStamp } }) => {
				...
				for (const [key, entry] of Object.entries(mutations)) if ((entry?.status === "fulfilled" || entry?.status === "rejected") && key !== entry?.requestId) draft[key] = entry;
			});
		}
	});
	...
	function removeCacheKeyFromTags(draft, queryCacheKey) {
		...
		for (const tag of existingTags) {
			...
			if (tagSubscriptions) draft.tags[tagType][tagId] = tagSubscriptions.filter((qc) => qc !== queryCacheKey);
		}
...
var initialSubState = { status: "uninitialized" };
...
function buildSelectors({ serializeQueryArgs, reducerPath, createSelector: createSelector2 }) {
	...
	function selectInvalidatedBy(state, tags) {
		...
		for (const tag of tags.filter(isNotNullish).map(expandTagDescription)) {
			...
			let invalidateSubscriptions = (tag.id !== void 0 ? provided[tag.id] : flatten(Object.values(provided))) ?? [];
	...
	function selectCachedArgsForQuery(state, queryName) {
		return Object.values(selectQueries(state)).filter((entry) => entry?.endpointName === queryName && entry.status !== "uninitialized").map((entry) => entry.originalArgs);
	}
	...
	return function baseCreateApi(options) {
		...
		const api = {
			...
			enhanceEndpoints({ addTagTypes, endpoints }) {
				...
				if (endpoints) for (const [endpointName, partialDefinition] of Object.entries(endpoints)) if (typeof partialDefinition === "function") partialDefinition(context.endpointDefinitions[endpointName]);
				else Object.assign(context.endpointDefinitions[endpointName] || {}, partialDefinition);
				return api;
			}
		};
...
var buildCacheCollectionHandler = ({ reducerPath, api, queryThunk, context, internalState, selectors: { selectQueryEntry, selectConfig }, getRunningQueryThunk, mwApi }) => {
	...
	function handleUnsubscribe(queryCacheKey, endpointName, api2, config) {
		const keepUnusedDataFor = context.endpointDefinitions[endpointName]?.keepUnusedDataFor ?? config.keepUnusedDataFor;
...
var buildCacheLifecycleHandler = ({ api, reducerPath, context, queryThunk, mutationThunk, internalState, selectors: { selectQueryEntry, selectApiState } }) => {
	...
	const handler = (action, mwApi, stateBefore) => {
		...
		if (queryThunk.pending.match(action)) checkForNewCacheKey(action.meta.arg.endpointName, cacheKey, action.meta.requestId, action.meta.arg.originalArgs);
		else if (api.internalActions.cacheEntriesUpserted.match(action)) for (const { queryDescription, value } of action.payload) {
			const { endpointName, originalArgs, queryCacheKey } = queryDescription;
			checkForNewCacheKey(endpointName, queryCacheKey, action.meta.requestId, originalArgs);
			resolveLifecycleEntry(queryCacheKey, value, {});
		}
		else if (mutationThunk.pending.match(action)) {
			if (mwApi.getState()[reducerPath].mutations[cacheKey]) handleNewKey(action.meta.arg.endpointName, action.meta.arg.originalArgs, cacheKey, mwApi, action.meta.requestId);
		} else if (isFulfilledThunk(action)) resolveLifecycleEntry(cacheKey, action.payload, action.meta.baseQueryMeta);
		else if (api.internalActions.removeQueryResult.match(action) || api.internalActions.removeMutationResult.match(action)) removeLifecycleEntry(cacheKey);
...
var buildInvalidationByTagsHandler = ({ reducerPath, context, context: { endpointDefinitions }, mutationThunk, queryThunk, api, assertTagType, refetchQuery, internalState }) => {
	...
	function hasPendingRequests(state) {
		...
		for (const cacheRecord of [queries, mutations]) for (const key in cacheRecord) if (cacheRecord[key]?.status === "pending") return true;
		return false;
	}
	function invalidateTags(newTags, mwApi) {
		...
		if (state.config.invalidationBehavior === "delayed" && hasPendingRequests(state)) return;
		...
		context.batch(() => {
			for (const queryCacheKey of subscriptions.keys()) {
				...
				if (values.some((sub) => sub[type] === true) || values.every((sub) => sub[type] === void 0) && state.config[type]) {
					...
					else if (querySubState.status !== "uninitialized") api2.dispatch(refetchQuery(querySubState));
				}
			}
		});
	}
...
//# sourceMappingURL=rtk-query.modern-BD4qpozQ.js.map